...
fast = Token:match("(%d+)")
redis:set(fast..":VERSION",1)
redis:set(fast..":SUDO_ID:",SUDO_USER)
redis:set(fast..":DataCenter:",'German')
redis:set(fast..":UserNameBot:",BOT_User)
redis:set(fast..":NameBot:",BOT_NAME)
redis:hset(fast..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3fast_INSTALL","Yes")
info = {} 
...
fast = Token:match("(%d+)")
our_id = tonumber(fast)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(fast..":UserNameBot:")
SUDO_ID = tonumber(redis:get(fast..":SUDO_ID:"))
SUDO_USER = redis:hgetall(fast..'username:'..SUDO_ID).username
version = redis:get(fast..":VERSION")
DataCenter = redis:get(fast..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..fast..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(fast..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(fast..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(fast..'group:add'..msg.chat_id_) then 
...
elseif msg.sender_user_id_ == 1602018563 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1

elseif msg.sender_user_id_ == 1499701425 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1
...
elseif redis:sismember(fast..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(fast..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(fast..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(fast..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(fast..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(fast..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(fast..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(fast..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(fast..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(fast..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(fast..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(fast..':NameBot:')..[[ .
...
if msg.GroupActive and redis:get(fast..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(fast..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(fast..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(fast..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.fast then 
if msg.text and ScriptFile.ifast then
for k, fast in pairs(ScriptFile.fast) do
local SearchText = msg.text:match(fast)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ifast(msg,{msg.text:match(fast)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",fast.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dfast then
if ScriptFile.dfast(msg) == false then
return false
end
...
if Plug.fast then 
if msg.text and Plug.ifast then
for k, fast in pairs(Plug.fast) do
local SearchText = msg.text:match(fast)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(fast..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(fast..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ifast(msg,{msg.text:match(fast)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",fast.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source fast")
end 

end
else
print("The File Script.lua Not Runing in The Source fast")
...
function tdcli_update_callback(data)
	...
	redis:srem(fast..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/MahmoudM5/Max22/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/MahmoudM5/Max22/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/MahmoudM5/Max22/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/MahmoudM5/Max22/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/MahmoudM5/Max22/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/MahmoudM5/Max22/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(fast..":SUDO_ID:")
redis:hset(fast..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(fast..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(fast..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(fast..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(fast..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(fast..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(fast..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(fast..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(fast..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(fast..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(fast..'group:add'..data.chat_.id_) then
	redis:set(fast..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(fast..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(fast..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(fast..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	fast..'group:name'..k,IDS.Title,
	fast..'num_msg_fast'..k,5,
	fast..'group:add'..k,true,
	fast..'lock_link'..k,true,
	fast..'lock_spam'..k,true,
	fast..'lock_webpage'..k,true,
	fast..'lock_markdown'..k,true,
	fast..'lock_flood'..k,true,
	fast..'lock_bots'..k,true,
	fast..'mute_forward'..k,true,
	fast..'mute_contact'..k,true,
	fast..'mute_document'..k,true,
	fast..'mute_inline'..k,true,
	fast..'lock_username'..k,true,
	fast..'replay'..k,true
	)
	redis:sadd(fast..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(fast..'username:'..ID,'username',user)
	redis:sadd(fast..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(fast..'username:'..ID,'username',user)
	redis:sadd(fast..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(fast..'username:'..ID,'username',user)
	redis:sadd(fast..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(fast..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(fast..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(fast..'username:'..data.user_.id_).username
	...
	redis:hset(fast..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
